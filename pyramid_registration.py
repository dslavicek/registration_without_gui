...
def pyramid_registration_with_scheduler(ref, sample, max_iters=30, mu=0.02,
                                        scale_factors=None, datatype=torch.float32,
                                        verbose=False, enable_translation=True,
                                        enable_rotation=True, enable_scaling=True,
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                     [int(max_iters * 0.33), int(max_iters * 0.67)], gamma=gamma,
                                                     last_epoch=-1)
    # tensor of identity matrices for later use
    I = torch.eye(3, device=used_device).repeat((batch_size, 1, 1))
    ref = ref.to(used_device)
