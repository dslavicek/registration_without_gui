...
def register_batches(input_dir, max_iters=30, mu=0.02, batch_size=8, loss_fcn='mse', masks=None, save=False):
    if masks is None:
    ...
    for i in range(0, len(files), batch_size * 2):
        ...
        results = pyramid_registration(ref_tens, sam_tens,mask_ref=masks_np[0], mask_sample=masks_np[1], mu=mu, loss_fcn=loss_fcn, max_iters=max_iters)
